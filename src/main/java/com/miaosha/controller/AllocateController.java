...
//这个request mapping是get post里的参数，而不是浏览器里的URL
//浏览器里的URL是前端router里配置的
...
//        int total = drugModelList.size();
//        if(index < 1 || (total % 10 == 0 && index > total /10) || (total % 10 != 0 && index > total/10 +1)) {
//            throw new BusinessException(EmBusinessError.PARAMETER_VALIDATION_ERROR);
//        }
//        if(index * 10 <= total) {
//            drugModelList = drugModelList.subList((index - 1) * 10, index * 10);
//        }else {
//            drugModelList = drugModelList.subList((index - 1) * 10, total);
//        }
    ...
    public CommonReturnType handleApplication(@RequestParam(name = "applyTable") String applyTable)throws BusinessException{
        //myNote RequestParam里的name要和我设置的axios中的属性名一样
...
//        System.out.println(testVOList.get(0).getDrugId());
//        System.out.println("sat表中记录数目为:"+ satService.countSatEntryNums());
...
//            System.out.println(applyResult.getResult());
            //TODO 可以修改applyResult的内容结构，让前端知道申请失败的原因
